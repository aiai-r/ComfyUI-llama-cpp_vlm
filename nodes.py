...
class llama_cpp_model_loader:
    ...
    @classmethod
    def IS_CHANGED(s, model, mmproj, chat_handler, n_ctx, vram_limit, image_min_tokens, image_max_tokens, 启用思考=False, load_mtp=False):
        custom_config = {
            "model": model,
            "mmproj": mmproj,
            "chat_handler":chat_handler,
            "启用思考": bool(启用思考),
            "load_mtp": bool(load_mtp),
            "n_ctx": n_ctx,
    ...
    def loadmodel(self, model, mmproj, chat_handler, n_ctx, vram_limit, image_min_tokens, image_max_tokens, 启用思考=False, load_mtp=False):
        custom_config = {
            "model": model,
            "mmproj": mmproj,
            "chat_handler":chat_handler,
...
class llama_cpp_instruct_adv:
    ...
    def process(self, llama_model, preset_prompt, custom_prompt, system_prompt, inference_mode, max_frames, max_size, seed, force_offload, save_states, 输出think块=False, unique_id="", parameters=None, images=None, queue_handler=None, cache_image_analysis=False):
        ...
        输出think块 = 输出think块[0] if isinstance(输出think块, list) else 输出think块
        ...
        if raw_image_list:
            ...
            # 2. IMAGES 模式：打平 list 里的所有 item，把所有图片合并在 1 次推理中
                ...
                clean_text = clean_thinking_text(raw_text, output_think_block=bool(输出think块))
